...
import { z } from 'zod'
import type { StartWorkerRequest, WorkerJob, WorkerStatus } from './contracts.js'
...
export function createWorkerBrokerServer(manager: JobManager): McpServer {
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        ...
        return success({ worker: summarize(job) }, `started worker ${job.job_id}`)
      } catch (error) {
  ...
  server.registerTool(
    ...
    {
      title: 'List workers',
      description: 'List persisted worker jobs, optionally filtered by lifecycle status.',
  ...
  server.registerTool(
    'get_worker_status',
    {
      ...
      description: 'Get current lifecycle and assignment metadata for one worker job.',
      inputSchema: { job_id: z.string().min(1) },
    },
    async ({ job_id: jobId }) => {
      try {
        const job = await manager.get(jobId)
        return success({ worker: summarize(job) }, `${job.job_id}: ${job.status}`)
      } catch (error) {
        return failure(error)
      }
    },
    ...
    async ({ job_id: jobId }) => {
      try {
        ...
        if (job.result === undefined) {
          return failure(new Error(`worker ${job.job_id} is ${job.status}; no terminal result yet`))
        }
        return success(
          { worker: summarize(job), result: job.result as unknown as Record<string, unknown> },
  ...
  server.registerTool(
    ...
    {
      title: 'Cancel worker',
      description: 'Cancel a queued or running worker and retain its terminal audit state.',
      inputSchema: { job_id: z.string().min(1) },
    },
    async ({ job_id: jobId }) => {
      try {
