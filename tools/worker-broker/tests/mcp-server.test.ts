...
class McpFixtureProvider implements WorkerProvider {
  ...
  async run(context: ProviderRunContext): Promise<ProviderOutcome> {
    ...
    await writeFile(path.join(context.worktree, 'src', 'mcp.txt'), 'mcp-smoke\n')
    return {
      exit_code: 0,
      signal: null,
      model_result: {
...
test('MCP exposes start, list, status, result, and cancel tools', async () => {
  ...
  try {
    ...
    const providerSchema = (
      startWorker?.inputSchema.properties?.provider as { enum?: string[] } | undefined
    )
    ...
    const listed = structured(
      await client.callTool({ name: 'list_workers', arguments: { status: 'completed' } }),
    )
    ...
    assert.equal((evidence.verification as Array<Record<string, unknown>>)[0]?.exit_code, 0)
  } finally {
    await client.close()
    await server.close()
