...
def frame(sw: int, sh: int, surface_aspect: float, zoom: float) -> tuple[float, float, float, float]:
    src = sw / sh
...
def stats(sw: int, sh: int, sa: float, zoom: float) -> dict:
    ...
    return {"srcWidthShown": max(vis_w, 0), "srcHeightShown": max(vis_h, 0), "matteFrac": max(matte, 0)}
...
def main() -> None:
    ...
    for gi, g in enumerate(sheets, 1):
        ...
        for r, (slug, img, z, st) in enumerate(g):
            ...
            dr.text((PAD, y + 24), f"{img.width}x{img.height}  ar={img.width / img.height:.2f}", fill=(170, 170, 175))
